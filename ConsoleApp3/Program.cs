...
namespace ConsoleApp3
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine(corPath2[0].x);
            Console.WriteLine(corPath1[0].x);
            ...
            {
                List<Coordinate> intersects = new List<Coordinate>();
                foreach(Coordinate x in p1)
                {
                    foreach(Coordinate y in p2)
                    {
                ...
                {
                    Console.WriteLine(Math.Abs(x.x) + Math.Abs(x.y));
                }
            }
            ...
            List<Coordinate> GetCorPath(List<string> y)
            {
                ...
                List<Coordinate> path = new List<Coordinate>{ new Coordinate() { x= 0, y = 0 } };
                foreach (string x in y)
                {
                    ...
                    for (var i = 0; i < currLenght; i++)
                    {
                        if(currWay == "L"||currWay=="R")
                            path.Add(new Coordinate() { x = addCor(currWay), y = path[path.Count-1].y });
                        else if(currWay == "D"||currWay == "U")
                            path.Add(new Coordinate() { x = path[path.Count - 1].x, y = addCor(currWay) });
                    }
                }
                return path;
            }
