...
namespace ConsoleApp3
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            List<int> currCor;
            List<List<int>> corPath2 = GetCorPath(path2a);
            List<List<int>> corPath1 = GetCorPath(path1a);
 

            Console.WriteLine(corPath2[0][0]);
            Console.WriteLine(corPath1[0][0]);

            Console.ReadLine();

            List<List<int>> GetCorPath(List<string> y)
            {
                currCor = new List<int> { 0, 0 };
                List<List<int>> path = new List<List<int>>{ new List<int> { 0,0 } };
                foreach (string x in y)
                {
                    ...
                    for (var i = 0; i < currLenght; i++)
                    {
                        if(currWay == "L"||currWay=="R")
                            path.Add(new List<int> { addCor(currWay), path[path.Count-1][1] });
                        else
                            path.Add(new List<int> { path[path.Count - 1][0], addCor(currWay) });
                    }
                }
                return path;
            }
